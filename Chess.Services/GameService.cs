...
namespace Chess.Services
{
	public class GameService : Service<Game>, IGameService
	{
		...
		public Task<bool> IsMoveOfBot(long gameId)
		{
			...
			var color = game.GameLogs.Last().Index % 2 != 0 ? Color.White : Color.Black;
			return Task.FromResult((color == Color.Black && game.Invitation.Acceptor.IsBot) || (color == Color.White && game.Invitation.Invitator.IsBot));
		}
		...
		{
			var result =
					_gameLogRepository.Query(log => log.Index == logId && log.Game.Invitation.Id == invitationId)
		...
		{
			return await Task.FromResult(_gameLogRepository.Query(log => log.Game.Invitation.Id == invitationId).Select().Count());
		}
	}
}

...
angular.module('app.main', [])
    ...
    .directive('loadingContainer', function () {
        return {
            ...
            scope: false,
            link: function (scope, element, attrs) {
                var loadingLayer = angular.element(' <div class="loading"> <img class="loading-bar" src="/img/loading.gif"> </div>');
...
angular.module('app.smartui', [])
    ...
    .directive('ngFormvalidation', function () {
    ...
    .directive('ngInitphonemask', function () {
        return {
            link: function (scope, element, attrs) {
                $(function () {
    ...
    .controller('ModalDialogController', [
        '$scope', '$rootScope', '$modal', 'ngProgress', function ($scope, $rootScope, $modal, ngProgress) {

            //console.log($scope.selector);

            //use function
            $scope.open = function (item) {
                // console.log($scope.extendedApi);
                //alert($scope.selecter);
                var modalInstance = $modal.open({
                    ...
                    resolve: {
                        ...
                        extendedApi: function () {
                            return $scope.extendedApi;  //parse
                        }
                    }
                });
                ...
                modalInstance.result.then(function (result) {
                    ...
                    //Handle model
                    if ($scope.selector) {
                        result = $scope.selector(result);
                    }

                    $scope.api.addItem(result, function () {
                        ngProgress.complete();
                        $.smallBox({
                            title: $scope.boxTitleSuccess,
                            content: "<i class='fa fa-clock-o'></i> <i>" + $scope.boxContentSuccess + "</i>",
                            color: "#296191",
                        ...
                        ngProgress.complete();
                        $.smallBox({
                            title: $scope.boxTitleFail,
                            content: "<i class='fa fa-clock-o'></i> <i>" + $scope.boxContentFail + "</i>",
                            color: "#FF0000",
    ...
    .controller('ModalInstanceController', function ($scope, $modalInstance, item, extendedApi) {

        $scope.item = item;
        $scope.extendedApi = extendedApi;
    ...
    .directive('modal', function () {
        return {
            ...
            scope: {
                ...
                size: '=size',
                boxTitleSuccess: '=boxTitleSuccess',
                boxTitleFail: '=boxTitleFail',
                boxContentSuccess: '=boxContentSuccess',
                boxContentFail: '=boxContentFail',
                item: '=item',
                // function
                selector: '=selector'
            },
            controller: 'ModalDialogController',
            template: '<a href="" ng-click="open(item)" class="btn btn-block btn-success btn-lg btn"><i class="glyphicon glyphicon-plus"></i>  </a>',
            link: function (scope) {
    ...
    .controller('AutocompliteDropdownController', function ($scope) {

        $scope.blur = function () {
            $scope.loading = false;
            if (!$scope.model)
                angular.element('#' + $scope.elementId).val('');
        };
        ...
        $scope.getItems = function (val) {
            ...
            var parameters = {};
            if ($scope.extendedFilters) {
                parameters = $scope.extendedFilters(); // By default gets empty array
            }

            var filter = $scope.filter;

            parameters['count'] = $scope.numberElements;
            parameters[filter] = val;

            var api = $scope.api.getItems(parameters);
    ...
    .directive('autocompliteDropdown', function () {
        return {
            restrict: 'E',
            scope: {
                elementId: '=elementId',
            ...
            controller: 'AutocompliteDropdownController',
            templateUrl: 'views/templates/autocompliteDropdownTemplate.html',
            link: function (scope) {
            }
        }
    })
